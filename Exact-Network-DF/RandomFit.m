function [stateOccupancyPattern,transitionForRoutesForClasses,fbStates,rbStates,possibleStatesTransitionForClasses,...
    defragStates,allPossibleTransitionsDF,allPossibleTransitionsDepart,connectionsForDefragStates]= RandomFit(DF,totalNumberOfSlots,bandwidthPerClass,routes)
...
% delete extra unused defrag states
 ...
 %outputTodefrag=[]; % if no fragmented state then output will give error
 %defragStates= index1; 

 %normalOrDefragOrSecurity = [normalOrDefragOrSecurity;ones(defragStates,1)];
 
        
   
   %transitionStatesForDefrag = [];
 % defragBlockingStates = zeros(defragStates,classes);
