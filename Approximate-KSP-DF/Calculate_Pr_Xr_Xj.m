function [Pr_Xr_Xj_c] = Calculate_Pr_Xr_Xj(bandwidthPerClass,freeSlotsPerState,PoA_link,stateProbabilities,numberOfLinks,path)
    ...
    for  state1=1:runLength(c)
         ...
         for p=1:numberOfLinks-1
             ...
             for state2=1:1:runLength(c)
                 factor1=factor1+stateProbabilities{path(p)}(state2)*PoA_link(state2,c);
             end
